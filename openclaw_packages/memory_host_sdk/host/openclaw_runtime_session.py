...
def is_heartbeat_user_message(message: dict, prompt: str) -> bool:
...
def list_session_entries(params: dict) -> list:
    return []
...
def is_exec_completion_event(message: dict) -> bool:
    content = message.get("content", "") if isinstance(message, dict) else ""
...
def has_inter_session_user_provenance(message: dict) -> bool:
