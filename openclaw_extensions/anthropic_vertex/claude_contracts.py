"""Claude model identity and effort helpers mirrored from @openclaw/llm-core anthropic contracts."""

from __future__ import annotations

import re
from typing import Any

CLAUDE_FABLE_5_THINKING_PROFILE: dict[str, Any] = {
    "levels": [
        {"id": "off"},
        {"id": "minimal"},
        {"id": "low"},
        {"id": "medium"},
        {"id": "high"},
        {"id": "xhigh"},
        {"id": "adaptive"},
        {"id": "max"},
    ],
    "defaultLevel": "high",
    "preserveWhenCatalogReasoningFalse": True,
}

_EXTENDED_THINKING_LEVELS = [
    "off",
    "minimal",
    "low",
    "medium",
    "high",
    "xhigh",
    "max",
]


def _normalize_claude_model_id(model_id: str | None) -> str:
    normalized = (model_id or "").strip().lower()
    unprefixed = normalized.removeprefix("anthropic/")
    return re.sub(r"[._\s]+", "-", unprefixed)


def resolve_claude_model_identity(ref: dict[str, Any] | Any) -> str:
    """Resolve the canonical normalized Claude model id for one runtime model ref."""
    if isinstance(ref, dict):
        params = ref.get("params") if isinstance(ref.get("params"), dict) else {}
        configured = params.get("canonicalModelId")
        model_id = ref.get("id")
    else:
        params = getattr(ref, "params", None)
        configured = params.get("canonicalModelId") if isinstance(params, dict) else None
        model_id = getattr(ref, "id", None)

    normalized = _normalize_claude_model_id(
        configured if isinstance(configured, str) else str(model_id or "")
    )
    match = re.search(r"(?:^|[-/])claude-", normalized)
    if not match:
        return normalized
    start = match.start()
    if match.group(0).startswith("claude-"):
        return normalized[start:]
    return normalized[start + 1:]


def resolve_claude_fable5_model_identity(ref: dict[str, Any] | Any) -> str | None:
    """Resolve Claude Fable 5 through direct ids, cloud ids, or deployment metadata."""
    normalized = resolve_claude_model_identity(ref)
    match = re.search(r"(?:^|-)claude-fable-5(?=$|[^a-z0-9])", normalized)
    if not match:
        return None
    start = match.start()
    if match.group(0).startswith("-"):
        return normalized[start + 1:]
    return normalized[start:]


def supports_claude_adaptive_thinking(ref: dict[str, Any] | Any) -> bool:
    model_id = resolve_claude_model_identity(ref)
    return bool(
        re.search(
            r"(?:^|-)claude-(?:fable-5|mythos-preview|opus-4-(?:6|7|8)|sonnet-4-6)(?=$|[^a-z0-9])",
            model_id,
        )
    )


def supports_claude_native_max_effort(ref: dict[str, Any] | Any) -> bool:
    model_id = resolve_claude_model_identity(ref)
    return bool(
        re.search(
            r"(?:^|-)claude-(?:fable-5|opus-4-(?:6|7|8)|sonnet-4-6)(?=$|[^a-z0-9])",
            model_id,
        )
    )


def supports_claude_native_xhigh_effort(ref: dict[str, Any] | Any) -> bool:
    model_id = resolve_claude_model_identity(ref)
    return bool(
        re.search(r"(?:^|-)claude-(?:fable-5|opus-4-(?:7|8))(?=$|[^a-z0-9])", model_id)
    )


def resolve_claude_native_thinking_level_map(
    ref: dict[str, Any] | Any,
) -> dict[str, str | None] | None:
    if isinstance(ref, dict):
        thinking_level_map = ref.get("thinkingLevelMap")
    else:
        thinking_level_map = getattr(ref, "thinkingLevelMap", None)
        if thinking_level_map is None:
            thinking_level_map = getattr(ref, "thinking_level_map", None)

    if thinking_level_map is not None:
        return thinking_level_map if isinstance(thinking_level_map, dict) else None
    if not supports_claude_native_max_effort(ref):
        return None
    return {
        "xhigh": "xhigh" if supports_claude_native_xhigh_effort(ref) else None,
        "max": "max",
    }


def get_supported_thinking_levels(model: dict[str, Any] | Any) -> list[str]:
    api = model.get("api") if isinstance(model, dict) else getattr(model, "api", None)
    fable_contract = api == "anthropic-messages" and resolve_claude_fable5_model_identity(model)
    reasoning = (
        model.get("reasoning") if isinstance(model, dict) else getattr(model, "reasoning", None)
    )
    if not reasoning and not fable_contract:
        return ["off"]

    thinking_level_map = (
        resolve_claude_native_thinking_level_map(model)
        if api == "anthropic-messages"
        else (
            model.get("thinkingLevelMap")
            if isinstance(model, dict)
            else getattr(model, "thinkingLevelMap", None)
        )
    )
    if thinking_level_map is None and isinstance(model, dict):
        thinking_level_map = model.get("thinking_level_map")

    levels: list[str] = []
    for level in _EXTENDED_THINKING_LEVELS:
        if isinstance(thinking_level_map, dict) and level in thinking_level_map and thinking_level_map[level] is None:
            continue
        if level in {"xhigh", "max"} and (
            not isinstance(thinking_level_map, dict) or level not in thinking_level_map
        ):
            continue
        levels.append(level)
    return levels


def clamp_thinking_level(model: dict[str, Any] | Any, level: str) -> str:
    """Clamp a requested thinking level to the closest supported level for a model."""
    available_levels = get_supported_thinking_levels(model)
    if level in available_levels:
        return level

    requested_index = _EXTENDED_THINKING_LEVELS.index(level) if level in _EXTENDED_THINKING_LEVELS else -1
    if requested_index == -1:
        return available_levels[0] if available_levels else "off"

    api = model.get("api") if isinstance(model, dict) else getattr(model, "api", None)
    thinking_level_map = (
        resolve_claude_native_thinking_level_map(model)
        if api == "anthropic-messages"
        else (
            model.get("thinkingLevelMap")
            if isinstance(model, dict)
            else getattr(model, "thinkingLevelMap", None)
        )
    )
    if thinking_level_map is None and isinstance(model, dict):
        thinking_level_map = model.get("thinking_level_map")

    if (
        level in {"xhigh", "max"}
        and isinstance(thinking_level_map, dict)
        and thinking_level_map.get(level) is None
    ):
        for index in range(requested_index - 1, -1, -1):
            candidate = _EXTENDED_THINKING_LEVELS[index]
            if candidate in available_levels:
                return candidate

    for index in range(requested_index, len(_EXTENDED_THINKING_LEVELS)):
        candidate = _EXTENDED_THINKING_LEVELS[index]
        if candidate in available_levels:
            return candidate
    for index in range(requested_index - 1, -1, -1):
        candidate = _EXTENDED_THINKING_LEVELS[index]
        if candidate in available_levels:
            return candidate
    return available_levels[0] if available_levels else "off"
